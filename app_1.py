...
st.set_page_config(page_title="Case - Novos Negócios (Opção 1: Session State)", layout="wide", page_icon="🚗")

# Inicialização da variável de estado para navegação
...
# Funções de formatação numérica no padrão brasileiro
def fmt_brl(val):
    if pd.isna(val):
        return "R$ 0,00"
...
def fmt_brl_int(val):
    if pd.isna(val):
        return "R$ 0"
    return "R$ " + f"{val:,.0f}".replace(',', '.')

def fmt_pct(val, dec=2):
    if pd.isna(val):
        return "0,00%"
    return f"{val * 100:.{dec}f}".replace('.', ',') + "%"
...
# ==========================================
# FUNÇÃO DA TELA DE APRESENTAÇÃO (SPLASH / CAPA)
# ==========================================
def render_splash_screen():
    img_b64 = get_base64_image("Porto_Holding_RGB_Horizontal-Cyan.webp")
    ...
    .splash-card {
        ...
        max-width: 820px;
        text-align: center;
        box-shadow: 0 15px 45px rgba(0, 45, 114, 0.08);
    ...
    .splash-divider {
        width: 70px;
        height: 4px;
        background: linear-gradient(90deg, #00A1FC, #004691);
        border-radius: 2px;
    ...
    if img_b64:
        ...
        <div class="splash-card">
            <img src="data:image/webp;base64,{img_b64}" class="splash-logo" alt="Logo Porto Seguro" />
            <h1 class="splash-slogan">Todo cuidado é Porto</h1>
            <div class="splash-divider"></div>
            <p class="splash-subtitle">Análise Estratégica de Precificação & Posicionamento de Mercado • Novos Negócios</p>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("""
        ...
        with col2:
            st.image("Porto_Holding_RGB_Horizontal-Cyan.webp", use_container_width=True)
    ...
    with col_btn2:
        if st.button("🚀 Iniciar Apresentação", type="primary", use_container_width=True):
            st.session_state.page = 'contexto'
            st.rerun()

# ==========================================
# FUNÇÃO DA TELA 2: CONTEXTO & DESAFIO
# ==========================================
def render_context_page():
    col_top1, col_top2 = st.columns([1, 5])
    with col_top1:
        if st.button("⬅️ Voltar à Capa", use_container_width=True):
            st.session_state.page = 'splash'
            st.rerun()

    st.image("Porto_Holding_RGB_Horizontal-Cyan.webp", width=250)
    st.markdown("""
    <div style="background-color: #00A1FC; padding: 35px 25px; border-radius: 15px; color: white; margin-bottom: 25px; text-align: center;">
        <h1 style="color: white; margin-bottom: 8px; font-weight: 700; font-size: 2.1rem;">🚗 Análise de Posicionamento - Novos Negócios</h1>
        <h3 style="color: #ffffff; font-weight: 400; margin-top: 0; margin-bottom: 12px; font-size: 1.25rem;">Estratégia de Precificação para atingimento de Meta de 15% de Conversão</h3>
        <p style="font-size: 1.05em; opacity: 0.95; margin: 0 auto; width: 100%;">
            Este painel apresenta o diagnóstico de precificação da Porto Seguro frente aos concorrentes no mercado de auto, a fim de propor um plano de ação para atingir a meta de vendas.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_ctx1:
        st.markdown("""
        <div style="background-color: #FFFFFF; border: 1px solid #E2E8F0; border-left: 6px solid #00A1FC; border-radius: 14px; padding: 28px 25px; min-height: 290px; box-shadow: 0 4px 15px rgba(0,0,0,0.04); display: flex; flex-direction: column; justify-content: flex-start;">
            <h4 style="color: #002D72; margin-top: 0; margin-bottom: 16px; font-weight: 700; font-size: 1.15rem;">📋 Contexto & Base de Dados</h4>
    ...
    with col_ctx2:
        st.markdown("""
        <div style="background-color: #FFFFFF; border: 1px solid #E2E8F0; border-left: 6px solid #002D72; border-radius: 14px; padding: 28px 25px; min-height: 290px; box-shadow: 0 4px 15px rgba(0,0,0,0.04); display: flex; flex-direction: column; justify-content: space-between;">
            <div>
                <h4 style="color: #002D72; margin-top: 0; margin-bottom: 16px; font-weight: 700; font-size: 1.15rem;">🎯 O que esperamos?</h4>
                <ul style="color: #334155; font-size: 1rem; line-height: 1.85; margin-bottom: 15px; padding-left: 22px;">
                    <li><b>Diagnóstico de Posicionamento:</b> Identificar a competitividade tarifária frente aos concorrentes.</li>
                    <li><b>Meta de 15% de Vendas:</b> O que precisamos fazer para atingir a meta no mês?</li>
                </ul>
            </div>
            <div style="background-color: #F8FAFC; border-radius: 8px; padding: 12px 16px; border: 1px dashed #CBD5E1; margin-top: 8px;">
                <p style="color: #334155; font-size: 0.92em; margin: 0; line-height: 1.4;">
                    💡 <b>Foco da Apresentação:</b> Plano de ajuste tarifário ótimo.
                </p>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    col_btn1, col_btn2, col_btn3 = st.columns([1, 1.4, 1])
    with col_btn2:
        if st.button("📊 Avançar para Diagnóstico & Métricas ➔", type="primary", use_container_width=True):
            st.session_state.page = 'diagnostico'
            st.rerun()

# ==========================================
# FUNÇÃO DA TELA 3: DIAGNÓSTICO EXECUTIVO (BIG NUMBERS & PILARES)
# ==========================================
def render_diagnostico_page():
    col_top1, col_top2 = st.columns([1, 5])
    with col_top1:
        if st.button("⬅️ Voltar ao Contexto", use_container_width=True):
            st.session_state.page = 'contexto'
            st.rerun()
    ...
    <div style="margin-bottom: 20px;">
        <h2 style="color: #002D72; margin-bottom: 5px; font-weight: 700;">📊 Diagnóstico de Conversão & Desempenho</h2>
        <p style="color: #64748B; font-size: 1.05rem; margin-top: 0;">Métricas Consolidadas de Share de Cotações e Conversão vs Meta</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 12px; padding: 20px; text-align: center;">
            ...
            <p style="color: #166534; margin: 5px 0 0 0; font-size: 0.82em; font-weight: 500;">Dominância no topo de funil em ambos os perfis</p>
        </div>
        """, unsafe_allow_html=True)
    with col2:
        st.markdown("""
    ...
    # Storytelling / Destaques do Diagnóstico
    st.subheader("📌 Diagnóstico Estratégico em 3 Pilares")
    ...
    with col_p3:
        st.markdown("""
        <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px; height: 100%;">
            <h4 style="color: #0f172a; margin-top: 0; margin-bottom: 12px; font-weight: 600;">3. Modelagem Data-Driven</h4>
            <p style="color: #334155; font-size: 0.95em; line-height: 1.6; margin: 0;">
                Aplicação de regressão linear (OLS), clusterização de competidores (K-Means) e projeção temporal de séries temporais.
            </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_btn2:
        if st.button("🚀 EXPLORAR DASHBOARD INTERATIVO COMPLETO", type="primary", use_container_width=True):
...
if st.session_state.page == 'splash':
...
elif st.session_state.page == 'contexto':
    render_context_page()

elif st.session_state.page in ['home', 'diagnostico']:
    render_diagnostico_page()

else:
    # BOTÕES NA SIDEBAR PARA VOLTAR
    st.sidebar.image("Porto_Holding_RGB_Horizontal-Cyan.webp")
    if st.sidebar.button("📊 Voltar ao Diagnóstico", use_container_width=True):
        st.session_state.page = 'diagnostico'
        st.rerun()
    if st.sidebar.button("📋 Voltar ao Contexto", use_container_width=True):
        st.session_state.page = 'contexto'
        st.rerun()
    if st.sidebar.button("🏠 Voltar à Capa", use_container_width=True):
        st.session_state.page = 'splash'
        st.rerun()
    ...
    with tab1:
        ...
        with col1:
            ...
            st.metric("Preço Médio Cia", fmt_brl(avg_price_cia))
        with col2:
            avg_price_mkt = df_filtered['Preco_Mercado'].mean()
            delta_mkt = (avg_price_cia / avg_price_mkt) - 1 if avg_price_mkt > 0 else 0
            st.metric("Preço Médio Mercado", fmt_brl(avg_price_mkt), f"{fmt_pct(delta_mkt, 1)} vs Cia", delta_color="inverse")
        with col3:
            avg_vendas = df_filtered['Vendas'].mean()
            st.metric("Média de Vendas", fmt_pct(avg_vendas, 2))
    ...
    with tab2:
        ...
        st.markdown("Nesta seção utilizamos K-Means para agrupar as **Seguradoras** de acordo com o **Preço Médio** e o **Volume Total de Cotações**.")
        ...
        if df_comp['Preco_Medio'].notna().all() and not df_comp.empty:
            ...
            st.info("💡 **Interpretação dos Grupos:** O K-Means avaliou Preço e Volume e identificou quem briga no mesmo território que nós.")
        else:
    ...
    with tab3:
        ...
        with subtab1:
            ...
            if len(df_sim_hist) > 0:
                ...
                try:
                    ...
                    st.info(f"De acordo com a tendência estatística, para atingir **15,0% de Vendas**, a relação do preço da Porto contra o Mercado deve ser de **{fmt_pct(preco_alvo_competitividade, 1)}**.")
                    ...
                    with col_sim1:
                        ...
                        st.success(f"**Preço Médio Sugerido para a Porto:** {fmt_brl_md(preco_sugerido)}")
                    ...
                    with col_sim2:
                        st.metric("Ajuste Médio Necessário", fmt_brl(diferenca), f"{texto_diff} vs médio praticado", delta_color="inverse" if diferenca < 0 else "normal")
                    with col_sim3:
                        st.metric("Fator de Ajuste (%)", fmt_pct(fator_ajuste, 2), "Ajuste na tarifa base", delta_color="inverse" if fator_ajuste < 0 else "normal")
                    ...
                    st.warning("Não foi possível calcular a projeção.")
            else:
        ...
        with subtab2:
            ...
            if len(df_trend_t3) >= 3 and len(df_filtered) > 2:
                ...
                st.info(f"A previsão estatística aponta que o Preço do Mercado em Setembro será de **{fmt_brl_md(preco_mercado_set)}**.")
                st.success(f"**Preço Alvo Estratégico (Setembro):** {fmt_brl_md(preco_sugerido_set)}")
                ...
                with col_met1:
                    st.metric("Esforço de Preço (Real vs Setembro)", fmt_brl(diferenca_set), delta_color="inverse" if diferenca_set < 0 else "normal")
                with col_met2:
                    st.metric("Fator de Ajuste Estimado (%)", fmt_pct(fator_ajuste_set, 2), delta_color="inverse" if fator_ajuste_set < 0 else "normal")
        ...
        with subtab3:
            ...
            if len(df_filtered) > 2:
                ...
                st.markdown(f"A cada **1%** de aumento no preço relativo, a Porto perde em média **{abs(slope):.2f} p.p.** na conversão de vendas.")
            else:
                st.warning("Dados insuficientes.")
    ...
    with tab4:
        ...
        if len(df_filtered) > 2:
            ...
            dados_plot.append({'Seguradora': 'Porto', 'Preco': preco_atual_porto, 'Tipo': 'Atual (Porto)', 'Rotulo': f"Atual: {fmt_brl_int(preco_atual_porto)}"})
            dados_plot.append({'Seguradora': 'Porto', 'Preco': preco_alvo_porto, 'Tipo': 'Simulado (Meta 15%)', 'Rotulo': f"Alvo: {fmt_brl_int(preco_alvo_porto)} ({fmt_pct(fator_ajuste_sim, 1)})"})
            dados_plot.append({'Seguradora': 'Média Mercado', 'Preco': mercado_base, 'Tipo': 'Mercado Médio', 'Rotulo': f"Média: {fmt_brl_int(mercado_base)}"})
            
            for i in range(1, 5):
                preco_conc = df_base[f'Preco_Concorrente_{i}'].iloc[0]
                dados_plot.append({'Seguradora': f'Concorrente {i}', 'Preco': preco_conc, 'Tipo': 'Congêneres', 'Rotulo': fmt_brl_int(preco_conc)})
            ...
            st.success(f"**Conclusão Estratégica:** Sob a ótica deste cenário, a Porto precisaria realizar uma **{texto_diff} de {fmt_brl_md(abs(diferenca_sim))}** no preço frente à sua média local, reposicionando-se para **{fmt_brl_md(preco_alvo_porto)}**.")
        else:
