...
st.set_page_config(page_title="Case - Novos Negócios (Opção 2: Aba Início)", layout="wide", page_icon="🚗")
...
# ==========================================
# TABS (COM A PRIMEIRA ABA SENDO A VISÃO GERAL)
# ==========================================
tab0, tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "🏠 Visão Geral & Contexto", 
    "🎯 Posicionamento de Mercado", 
    "🧩 Análise de Cluster", 
    "📈 Meta de 15% de Vendas", 
    "🔮 Cenário Simulado", 
    "🧠 Racional Matemático"
])
...
with tab0:
    st.markdown("""
    <div style="background: linear-gradient(135deg, #004691 0%, #00A1FC 100%); padding: 30px; border-radius: 12px; color: white; margin-bottom: 20px;">
        <h2 style="color: white; margin-bottom: 5px;">🚀 Bem-vindo ao Painel Estratégico de Novos Negócios</h2>
        <p style="font-size: 1.1em; opacity: 0.95; margin-top: 5px;">
            Este painel foi estruturado para responder como a Porto Seguro pode atingir a <b>Meta de 15% de Conversão de Vendas</b> ajustando estrategicamente seu posicionamento de preço frente aos concorrentes do mercado de auto.
        </p>
    </div>
    """, unsafe_allow_html=True)
    
    # Cards de Destaque Executivo com Métricas por Produto (Produto 1 vs Produto 2)
    ...
    st.subheader("💡 Como navegar neste Dashboard?")
    st.markdown("""
    Explore as abas no topo da tela para visualizar cada etapa da análise:
    
    * **🎯 Posicionamento de Mercado:** Gráficos de evolução temporal de preços (com projeção para Setembro), conversão de vendas e scatter plot de elasticidade.
    * **🧩 Análise de Cluster:** Agrupamento por Inteligência Artificial (K-Means) identificando em qual grupo de concorrentes a Porto está alocada.
    * **📈 Meta de 15% de Vendas:** Simuladores de curva de preço considerando o histórico, a projeção preditiva de Setembro e a inclinação de elasticidade.
    * **🔮 Cenário Simulado:** Comparador visual do prêmio alvo da Porto vs Congêneres sob 5 óticas estatísticas distintas.
    * **🧠 Racional Matemático:** Explicação completa das metodologias (Regressão OLS, K-Means e Séries Temporais).
    """)
    st.info("👈 Use a barra lateral à esquerda para filtrar por Perfil de Cliente (Produto 1 ou Produto 2) ou por Períodos de tempo específicos.")
...
with tab3:
    ...
    with subtab1:
        ...
        if len(df_sim_hist) > 0:
            ...
            try:
                ...
                st.info(f"De acordo com a tendência estatística, para atingir **15% de Vendas**, a relação do preço da Porto contra o Mercado deve ser de **{preco_alvo_competitividade:.1%}**.")
    ...
    with subtab2:
        ...
        if len(df_trend_t3) >= 3 and len(df_filtered) > 2:
            ...
            st.info(f"A previsão estatística aponta que o Preço do Mercado em Setembro será de **R$ {preco_mercado_set:,.2f}**.")
            ...
            with col_met1:
                st.metric("Esforço de Preço (Real vs Setembro)", f"R$ {diferenca_set:,.2f}", delta_color="inverse" if diferenca_set < 0 else "normal")
            with col_met2:
...
with tab4:
    ...
    if len(df_filtered) > 2:
        ...
        st.success(f"**Conclusão Estratégica:** Sob a ótica deste cenário, a Porto precisaria realizar uma **{texto_diff} de R$ {abs(diferenca_sim):.2f}** no preço frente à sua média local, reposicionando-se para **R$ {preco_alvo_porto:,.2f}**.")
    else:
...
with tab5:
    ...
    Mapeamento por Regressão OLS (`y = ax + b`) correlacionando Competitividade com Vendas.
    
    ---
    ### 2. Mapa de Competição (Algoritmo K-Means)
    Clusterização não supervisionada com `StandardScaler` e `KMeans(n_clusters=3)`.
    
    ---
    ### 3. Previsão de Setembro (Projeção de Séries Temporais)
    Extrapolação linear temporal de tendência de preços e cotações.
    
    ---
    ### 4. Coeficiente de Elasticidade-Preço da Demanda (Epd)
    Identificação da sensibilidade de conversão em pontos percentuais por 1% de alteração de preço.
    """)
